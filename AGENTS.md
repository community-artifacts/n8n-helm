...
- **Branch flow.** `dev/<topic>` → `develop` → `main`. Direct push to both `develop` and `main` is blocked by branch protection for humans (the only allowed exception on `develop` is the `github-actions[bot]` running `version-bump.yml`). `dev/**` and `develop` trigger `.github/workflows/validate.yml` (lint + schema + unit tests + scenario matrix + kubeconform + minikube). Opening a PR `develop` → `main` triggers `.github/workflows/version-bump.yml`, which computes the next chart version from the Conventional-Commits log and pushes the bump + RELEASE-NOTES stub + regenerated `artifacthub.io/changes` back onto `develop`. Merging that PR triggers `.github/workflows/release.yml` (chart-releaser-action: package, sign, publish to `gh-pages`, create GitHub Release). Releases happen exclusively via merged PR `develop` → `main`. See [CONTRIBUTING.md](CONTRIBUTING.md#branch-strategy) and [TESTING.md](TESTING.md).
