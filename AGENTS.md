...
- **Branch flow.** `dev/<topic>` → `develop` → `main`. Direct push to both `develop` and `main` is blocked by branch protection for humans (the only allowed exception on `develop` is the `github-actions[bot]` running the bump / scheduled-release / hotfix-release workflows). `dev/**` and `develop` trigger `.github/workflows/validate.yml` (lint + schema + unit tests + scenario matrix + kubeconform + minikube). Releases reach `main` three ways, all using the [Bumpy](improvements/bumpy.md) quantity-based SemVer strategy via `scripts/bumpy_decide.sh` (MAJOR is capped at MINOR — chart MAJOR is pinned to n8n's MAJOR): (a) **nightly cron** — `.github/workflows/scheduled-release.yml` runs at 02:00 UTC, opens the release PR if develop is ahead, enables auto-merge; (b) **hotfix** — `.github/workflows/hotfix-release.yml` triggers immediately on any push to `develop` whose tip commit subject matches a hotfix marker (`hotfix:` / `hotfix(<scope>):` / `fix!:` with `hotfix` in the body / `[HOTFIX]` tag), runs the same release path without waiting for the cron; (c) **manual** — opening the PR `develop` → `main` yourself triggers `.github/workflows/version-bump.yml`, which uses Conventional Commits to pick the bump and pushes the result onto `develop`. In all three cases the merge to `main` triggers `.github/workflows/release.yml` (chart-releaser-action: package, sign, publish to `gh-pages`, create GitHub Release). See [CONTRIBUTING.md](CONTRIBUTING.md#branch-strategy) and [TESTING.md](TESTING.md).
