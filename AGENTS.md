...
- **Branch flow.** `dev/<topic>` → `develop` → `main`. Direct push to `main` is blocked by branch protection. `dev/**` and `develop` trigger `.github/workflows/validate.yml` (lint + schema + unit tests + scenario matrix + kubeconform + minikube). `main` triggers `.github/workflows/release.yml` (chart-releaser-action: package, sign, publish to `gh-pages`, create GitHub Release). Releases happen exclusively via merged PR `develop` → `main`. See [CONTRIBUTING.md](CONTRIBUTING.md#branch-strategy) and [TESTING.md](TESTING.md).
