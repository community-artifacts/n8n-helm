...
- **Branch flow.** `dev/<topic>` → `develop` → `main`. Direct push to both `develop` and `main` is blocked by branch protection for humans (the only allowed exception on `develop` is the `github-actions[bot]` running the bump / scheduled-release workflows). `dev/**` and `develop` trigger `.github/workflows/validate.yml` (lint + schema + unit tests + scenario matrix + kubeconform + minikube). Releases reach `main` two ways: (a) **nightly cron** — `.github/workflows/scheduled-release.yml` runs at 02:00 UTC, counts commits on `develop` ahead of `main`, picks MINOR if `>4` else PATCH, bumps the chart, opens the release PR with the `bot/release` label, and enables auto-merge. (b) **manual** — opening the PR `develop` → `main` yourself triggers `.github/workflows/version-bump.yml`, which computes the bump from Conventional Commits and pushes the result onto `develop`. Either way, merge to `main` triggers `.github/workflows/release.yml` (chart-releaser-action: package, sign, publish to `gh-pages`, create GitHub Release). See [CONTRIBUTING.md](CONTRIBUTING.md#branch-strategy) and [TESTING.md](TESTING.md).
